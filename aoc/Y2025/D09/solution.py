...
def part_1(red_tiles):
    return max(Rectangle(r1, r2).area(padding=Point(1, 1)) for r1, r2 in itertools.combinations(red_tiles, 2))

def part_2(red_tiles):
    paths = consecutive_pairs(red_tiles, True)

    return max(Rectangle(r1, r2).area(padding=Point(1, 1)) for r1, r2 in itertools.combinations(red_tiles, 2)
               if all(not (r := Rectangle(r1, r2)).intersects(Rectangle(p1, p2), 2, allowable_overlap=1) for p1, p2 in paths))